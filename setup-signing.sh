#!/usr/bin/env bash
#
# Create a stable self-signed code-signing certificate for Hush.
#
...
# once and it survives every rebuild/update.
#
# Run this ONCE. Afterwards, ./build-app.sh signs with this identity automatically.
#
set -euo pipefail

CERT_NAME="Hush Self-Signed"
...
openssl pkcs12 -export -inkey "$TMP/key.pem" -in "$TMP/cert.pem" \
  -out "$TMP/cert.p12" -passout pass:hush -name "$CERT_NAME" >/dev/null 2>&1

echo "==> Importing into your login keychain..."
security import "$TMP/cert.p12" -k "$KEYCHAIN" -P hush -T /usr/bin/codesign >/dev/null
...
if security find-identity -v -p codesigning | grep -q "$CERT_NAME"; then
  ...
  echo "Now run ./build-app.sh && ./install.sh, then grant Accessibility ONCE. It will persist."
else
