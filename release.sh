#!/usr/bin/env bash
#
# Cut a GitHub release: build the signed app, package a DMG, and publish it.
# Usage: ./release.sh [version]     (version defaults to the VERSION file)
#
# Requires the `gh` CLI, authenticated (gh auth login).
#
set -euo pipefail

APP_NAME="Hush"

if [ -f "VERSION" ]; then
  # shellcheck disable=SC1091
  source "./VERSION"
fi
VERSION="${1:-${MARKETING_VERSION:-1.5.0}}"
TAG="v${VERSION}"
DMG="build/${APP_NAME}-${VERSION}.dmg"

command -v gh >/dev/null || {
  echo "gh CLI not found. Install: brew install gh" >&2
  exit 1
}

echo "==> Building DMG for ${TAG}..."
./build-dmg.sh "$VERSION"

# Release notes in a temp file (quoted heredoc = literal), then fill placeholders.
NOTES="$(mktemp)"
trap 'rm -f "$NOTES"' EXIT
cat > "$NOTES" <<'NOTESEOF'
## Hush __TAG__

Automatically quits macOS apps when their last window is closed, while keeping
background-only apps (Docker, Dropbox, …) running.

### Install
1. Download `__APP__-__VERSION__.dmg` below and open it.
2. Drag **__APP__** to Applications.
3. Hush is self-signed (not notarized), so clear the quarantine flag:
   ```
   xattr -dr com.apple.quarantine /Applications/__APP__.app
   ```
4. Launch it and grant **Accessibility** when prompted.
NOTESEOF

sed -i '' \
  -e "s/__TAG__/${TAG}/g" \
  -e "s/__APP__/${APP_NAME}/g" \
  -e "s/__VERSION__/${VERSION}/g" \
  "$NOTES"

if gh release view "$TAG" >/dev/null 2>&1; then
  echo "==> Release ${TAG} already exists — uploading DMG (clobber)..."
  gh release upload "$TAG" "$DMG" --clobber
else
  echo "==> Creating GitHub release ${TAG}..."
  gh release create "$TAG" "$DMG" \
    --title "${APP_NAME} ${TAG}" \
    --notes-file "$NOTES"
fi

echo
echo "Published: $TAG"
echo "Verify: gh release view $TAG"
